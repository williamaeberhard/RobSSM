NP_st_nrcorrect <- function(theta.t=NULL,obj,H=100,seedval=NULL,
                            ini.cond.fixed=F,trim=0,onlyimprove=T,
  ...
  theta.upd <- NP_st_untransfo(theta.t.upd)
  
  # extract predicted ini logNat and logFat on original data, fixed for simul
  if (ini.cond.fixed){ # fix logN1 and logF1 to predicted values in obj
  ...
  if (correct.hessian){
    ...
    stdist.h <- NP_st_stdist(theta=theta.upd) # do it once outside loop
    for (h in 1:H){
      samlist.h <- NP_st_gen(theta=theta.upd,ntrunc=newobj$env$data$ntrunc,
    ...
    Egrad <- apply(gradmat,2,mean,na.rm=T,trim=trim)/TF
